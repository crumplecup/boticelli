...
The Botticelli TUI is an interactive terminal interface for managing content generated by AI narratives. It provides a fast, keyboard-driven workflow for:
...
- Botticelli compiled with `--features tui` or `--all-features`
...
./target/debug/botticelli tui --help
```
...
botticelli tui guilds_generation_001
```
...
```
┌─────────────────────────────────────────────────────────┐
│ Botticelli Content Review - guilds_generation_001       │ ← Header
├─────────────────────────────────────────────────────────┤
│                                                         │
│                                                         │
...
```bash
# 1. Generate content
botticelli run --narrative weekly_posts.toml

# 2. Launch TUI
botticelli tui weekly_posts_001
...
botticelli content promote weekly_posts_001 123 --target discord_messages
```
...
botticelli run --narrative announcement_variants.toml

# 2. Launch TUI
botticelli tui announcement_variants
...
# 4. Tag the winner
botticelli content tag announcement_variants 3 --tags "winner,final"

# 5. Promote the winner
botticelli content promote announcement_variants 3 --target discord_messages
```
...
```bash
# 1. Launch TUI
botticelli tui test_generation_042
...
```bash
# 1. Launch TUI
botticelli tui channel_ideas

# 2. For each item:
#    - Press Enter to view details
#    - Read content carefully
#    - Press e to edit
#    - (Use CLI for now: botticelli content tag channel_ideas 123 --tags "excellent" --rating 5)
#    - Press Esc to return
#    - Navigate to next with ↓

# 3. Export high-rated items
botticelli content list channel_ideas --status approved --limit 1000 > approved.json
```
...
Set in `.env` file:
```bash
DATABASE_URL=postgres://user:pass@localhost:5432/botticelli
```
...
botticelli content tag your_table 123 --tags "tag1,tag2" --rating 4

# Edit review status
botticelli content review your_table 123 approved
```
...
- For larger datasets, use CLI filtering:
  ...
  botticelli content list your_table --status pending
  ```
...
DATABASE_NAME=botticelli_test botticelli tui my_table

# Use remote database
DATABASE_HOST=db.example.com DATABASE_PORT=5433 botticelli tui my_table
```
...
```bash
# Terminal 1
botticelli tui guilds_gen_001

# Terminal 2
botticelli tui channels_gen_002
```
...
botticelli content list my_table --status approved > approved.json

# Process with jq
cat approved.json | jq '.[] | select(.rating >= 4)'

# Import into spreadsheet
botticelli content list my_table --status approved | csv2xlsx > report.xlsx
```
...
Example:
```bash
botticelli content tag my_table 123 --tags "excellent,viral,ready,urgent" --rating 5
```
...
botticelli content list my_table --limit 10000 > backup_$(date +%Y%m%d).json

# Now safe to delete in TUI
botticelli tui my_table
```
...
- **CLI Help**: `botticelli tui --help`
- **Issues**: https://github.com/crumplecup/botticelli/issues
- **Discussions**: https://github.com/crumplecup/botticelli/discussions

## License

Botticelli is dual-licensed under MIT OR Apache-2.0.
