...
**`potential_posts` table** - Created as template for auto-storage:

- Used by generation narratives via `template = "potential_posts"`
- Matches `content` table schema
- Acts as staging area for generated content

**`content` table** - Used for curation and posting workflows:

- Approved posts copied/moved from `potential_posts`
- Tag-based workflow stages: `["potential"]` → `["approved"]` → posted

```sql
-- potential_posts table (template-based auto-storage)
CREATE TABLE potential_posts (
    ...
    metadata JSONB
    -- Other fields match content table
);

-- Workflow stages via tags:
--   ["potential"] - Generated by carousel, awaiting curation
--   ["approved"] - Curated and ready for posting
--   ["posted"] - Already posted to Discord (post_count > 0)

-- Query examples:
-- Potential posts: SELECT * FROM potential_posts WHERE 'potential' = ANY(tags)
-- For curation:    SELECT * FROM potential_posts ORDER BY created_at DESC LIMIT 10
-- Approved posts:  SELECT * FROM content WHERE 'approved' = ANY(tags) AND post_count = 0
-- Posted content:  SELECT * FROM content WHERE post_count > 0
```
...
2. Create `potential_posts` table template:
   - Matches `content` table schema
   - Used by `template = "potential_posts"` in narratives
   - Auto-storage system parses JSON and inserts rows
3. Add helper queries to `botticelli_database`:
...
4. Add bot commands for database operations:
...
- `crates/botticelli_database/src/content_queries.rs` (new)
- `crates/botticelli_social/src/commands/database.rs` (new)
- `crates/botticelli_narrative/src/templates/potential_posts.rs` (new)

**Table template definition:**

```rust
// potential_posts matches content table schema
CREATE TABLE potential_posts (
    id SERIAL PRIMARY KEY,
    content_type VARCHAR(50) NOT NULL,
    text_content TEXT,
...
### Phase 3: Curation Actor (Day 4-5)

**Tasks:**

1. Create `curation_pipeline.toml` narrative
2. Implement analysis and scoring logic
3. Add JSON parsing for scores
4. Test with real potential posts
5. Create `curation_actor.toml` config
6. Deploy and run for 2-3 days

**Files to create:**

- `crates/botticelli_narrative/narratives/discord/curation_pipeline.toml`
- `actors/curation_actor.toml`

**Success criteria:**

- Selects 2-3 best posts per run
- Approved posts measurably better than originals
- Maintains diversity in selected posts
