...
impl Budget {
    ...
    pub fn consume(&mut self, tokens: u64) -> Result<(), RateLimitError> {
        if !self.can_afford(tokens) {
            return Err(RateLimitError::new(RateLimitErrorKind::BudgetExceeded {
                ...
                available_tokens_minute: self.config.tokens_per_minute.saturating_sub(self.tokens_per_minute),
                available_tokens_day: self.config.tokens_per_day.saturating_sub(self.tokens_per_day),
                available_requests_minute: self.config.requests_per_minute.saturating_sub(self.requests_per_minute),
                available_requests_day: self.config.requests_per_day.saturating_sub(self.requests_per_day),
            }));
        }
    ...
    pub fn remaining(&mut self) -> BudgetRemaining {
        ...
        BudgetRemaining {
            tokens_per_minute: self.config.tokens_per_minute.saturating_sub(self.tokens_per_minute),
            tokens_per_day: self.config.tokens_per_day.saturating_sub(self.tokens_per_day),
            requests_per_minute: self.config.requests_per_minute.saturating_sub(self.requests_per_minute),
            requests_per_day: self.config.requests_per_day.saturating_sub(self.requests_per_day),
        }
    }
}
