...
fn test_get_last_successful() {
    ...
    repo.start_generation(create_test_generation("test_last_1"))
        .unwrap();
    std::thread::sleep(std::time::Duration::from_millis(100));

    repo.start_generation(create_test_generation("test_last_2"))
        .unwrap();
    std::thread::sleep(std::time::Duration::from_millis(100));

    repo.start_generation(create_test_generation("test_last_3"))
        .unwrap();
    ...
    repo.complete_generation("test_last_1", success_update.clone())
        .unwrap();
    std::thread::sleep(std::time::Duration::from_millis(100));
    repo.complete_generation("test_last_2", success_update.clone())
        .unwrap();
...
fn test_list_generations() {
    ...
    repo.start_generation(create_test_generation("test_list_1"))
        .unwrap();
    repo.start_generation(create_test_generation("test_list_2"))
        .unwrap();
    repo.start_generation(create_test_generation("test_list_3"))
        .unwrap();
    ...
    repo.complete_generation("test_list_1", success_update)
        .unwrap();
    ...
    repo.complete_generation("test_list_2", failed_update)
        .unwrap();

    // List all
    let all = repo.list_generations(None, 10).unwrap();
    let test_gens: Vec<_> = all
        .iter()
    ...
    let successful = repo
        .list_generations(Some("success".to_string()), 10)
        .unwrap();
    let test_success: Vec<_> = successful
        .iter()
        .filter(|g| g.table_name.starts_with("test_list_"))
        .collect();
    ...
    // List only failed
    let failed = repo
        .list_generations(Some("failed".to_string()), 10)
        .unwrap();
    let test_failed: Vec<_> = failed
        .iter()
        .filter(|g| g.table_name.starts_with("test_list_"))
        .collect();
...
fn test_list_with_limit() {
    ...
    // Create multiple generations
    repo.start_generation(create_test_generation("test_limit_1"))
        .unwrap();
    repo.start_generation(create_test_generation("test_limit_2"))
        .unwrap();
    repo.start_generation(create_test_generation("test_limit_3"))
        .unwrap();
